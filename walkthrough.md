...
## 7. Custom Instruments ("Others") Registration & Admin Notification Fix

#### [MODIFY] [composer.js](file:///c:/Users/gutis/Desktop/proyecto/js/composer.js)
- **RLS Bypass for Custom Instruments**: Replaced direct inserts into the `instruments` table (which failed due to RLS) with a local-only pending state. Custom instruments are now created with a temporary `custom_` prefix and selected as pending tags (marked with a ⏳ icon).
- **Admin Notification**: Triggered `notifyAdminsOfUnmatchedInstruments` immediately upon custom instrument tag selection. This sends a real-time message to all curators/admins detailing the requested instrument name and the affected work title.
- **Relational Integrity**: Modified `saveWork` to filter out any IDs starting with `custom_` before attempting to insert rows into `work_instruments`, preventing SQL foreign key constraint violations.
- **Instrumentation Text Serialization**: Constructed and saved the full, comma-separated list of selected instruments (both standard and pending custom ones) inside the `performer_combination` field of the `works` table.
