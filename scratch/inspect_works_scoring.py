...
for w in works[:20]:
    ...
    print(f"ID {w['id']} | Title: '{w['title']}'")
    print(f"  Scoring: {w.get('scoring_category')} | NumPerformers: {w.get('num_performers')} | Soloist: {w.get('soloist_instrument')}")
    print(f"  Instruments linked: {insts}")
    print("-" * 50)
